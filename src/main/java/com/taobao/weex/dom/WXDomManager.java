...
import com.taobao.weex.dom.action.AddRuleAction;
...
/**
 * Class for managing dom operation. This class works as the client in the command pattern, it will
 * call {@link DOMActionContextImpl} for creating command object and invoking corresponding
 * operation. Methods in this class normally need to be invoked in dom thread, otherwise, {@link
...
public final class WXDomManager {
    ...
    public void post(Runnable task) {
        if (mDomHandler == null || task == null || mDomThread == null || !mDomThread
                .isWXThreadAlive()
    ...
    private boolean isDomThread() {
        return !WXEnvironment.isApkDebugable() || Thread.currentThread().getId() == mDomThread
                .getId();
    }
    ...
    void batch() {
        throwIfNotDomThread();
        Iterator<Entry<String, DOMActionContextImpl>> iterator = mDomRegistries.entrySet()
                .iterator();
    ...
    public void executeAction(String instanceId, DOMAction action, boolean createContext) {
        if (action instanceof AddRuleAction) {
            createContext = true;
        }
        DOMActionContext context = mDomRegistries.get(instanceId);
        if (context == null) {
            if (createContext) {
                DOMActionContextImpl oldStatement = new DOMActionContextImpl(instanceId,
                        mWXRenderManager);
