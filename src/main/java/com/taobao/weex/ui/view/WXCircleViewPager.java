...
public class WXCircleViewPager extends ViewPager implements WXGestureObservable,
    ...
    private void postMotionEventToParent(MotionEvent ev) {
        int x = (int) ev.getX();
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mFlag) {
                    if (x - mLastX > 5 && mSelectPosition == 0) {
                        mFlag = false;
                        this.requestDisallowInterceptTouchEvent(false);
                    }

                    if (x - mLastX < -5 && getItemCount()-1 == mSelectPosition) {
                        mFlag = false;
                        this.requestDisallowInterceptTouchEvent(false);
                    }
                }
    ...
    private WeakReference<WXSlider> mWeakReference;
    private WXSlider mWXSlide;
    private BMSliderOnPageChangeListener mOnPageListener;
    private int mSelectPosition;
    private int mLastX;
    private boolean mFlag;

    private int getItemCount() {
        WXCirclePageAdapter circlePageAdapter = getCirclePageAdapter();
        if (circlePageAdapter != null) {
            return circlePageAdapter.getCount();
        }
        return 0;
    }

    @Override
    public void holdComponent(WXSlider component) {
        mWeakReference = new WeakReference<>(component);
        mWXSlide = mWeakReference.get();
        mOnPageListener = new BMSliderOnPageChangeListener();
        this.addOnPageChangeListener(mOnPageListener);
    }

    private class BMSliderOnPageChangeListener implements ViewPager.OnPageChangeListener {
