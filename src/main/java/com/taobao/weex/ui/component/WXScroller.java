...
public class WXScroller extends WXVContainer<ViewGroup> implements WXScrollViewListener,
    ...
    private boolean mAddCustomRefresh = false;
     ...
     * 添加在定义刷新头
     */
    private void addCustomeRefresh() {
        if (!mAddCustomRefresh || mBMRefresh != null) return;
        mBMRefresh = new BMLoadingRefresh(getContext(), this);
        ((BaseBounceView) getHostView()).setOnRefreshListener(mBMRefresh);
        getHostView().postDelayed(new Runnable() {
            @Override
            public void run() {
                ((BaseBounceView) getHostView()).setCustomHeaderView(mBMRefresh);
            }
        }, 100);
    }
    ...
    protected boolean setProperty(String key, Object param) {
        switch (key) {
            ...
            case Constants.Name.OFFSET_ACCURACY:
                ...
                return true;
            case Constants.Name.SHOW_REFRESH:
                mAddCustomRefresh = WXUtils.getBoolean(param, false);
                return true;
