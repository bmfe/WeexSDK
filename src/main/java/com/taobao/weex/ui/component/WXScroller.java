...
/**
 * Component for scroller. It also support features like "appear", "disappear" and "sticky"
 */
@Component(lazyload = false)

public class WXScroller extends WXVContainer<ViewGroup> implements WXScrollViewListener,
        Scrollable {
    ...
    @Deprecated
    public WXScroller(WXSDKInstance instance, WXDomObject dom, WXVContainer parent, String
    ...
    public void addEvent(String type) {
        super.addEvent(type);
        if (Constants.Event.SCROLL.equals(type) && getInnerView() != null && getInnerView()
                instanceof WXScrollView) {
            ((WXScrollView) getInnerView()).addScrollViewListener(new WXScrollViewListener() {
                @Override
                public void onScrollChanged(WXScrollView scrollView, int x, int y, int oldx, int
                        oldy) {
                    if (shouldReport(x, y)) {
                        ...
                        contentSize.put(Constants.Name.WIDTH, WXViewUtils.getWebPxByWidth(frame
                                .width(), viewport));
                        contentSize.put(Constants.Name.HEIGHT, WXViewUtils.getWebPxByWidth(frame
                                .height(), viewport));

                        contentOffset.put(Constants.Name.X, -WXViewUtils.getWebPxByWidth(x,
                                viewport));
                        contentOffset.put(Constants.Name.Y, -WXViewUtils.getWebPxByWidth(y,
                                viewport));
        ...
        } else if (Constants.Event.SCROLL.equals(type) && getInnerView() != null && getInnerView
                () instanceof WXHorizontalScrollView) {
            ((WXHorizontalScrollView) getInnerView()).setScrollViewListener(new WXHorizontalScrollView.ScrollViewListener() {


                @Override
                public void onScrollChanged(WXHorizontalScrollView scrollView, int x, int y, int
                        oldx,
                                            int oldy) {
                    if (shouldReport(x, y)) {
                        Rect frame = scrollView.getContentFrame();
    ...
    protected MeasureOutput measure(int width, int height) {
        ...
        if (this.mOrientation == Constants.Orientation.HORIZONTAL) {
            ...
            measureOutput.width = width > (weexW >= screenW ? screenW : weexW) ? FrameLayout
            ...
            measureOutput.height = height > (weexH >= screenH ? screenH : weexH) ? FrameLayout
                    .LayoutParams.MATCH_PARENT
                    : height;
    ...
    protected ViewGroup initComponentHostView(@NonNull Context context) {
            ...
            innerView.addScrollViewListener(new WXScrollViewListener() {
                ...
                public void onScrollStopped(WXScrollView scrollView, int x, int y) {
                    ...
                    if (listeners != null && listeners.size() > 0) {
                        for (OnWXScrollListener listener : listeners) {
                            if (listener != null) {
                                listener.onScrollStateChanged(scrollView, x, y,
                                        OnWXScrollListener.IDLE, getRef());
                            }
                        }
                    }
                }
                ...
                public void onScroll(WXScrollView scrollView, int x, int y) {
                    ...
                    if (listeners != null && listeners.size() > 0) {
                        for (OnWXScrollListener listener : listeners) {
                            if (listener != null) {
                                listener.onScrolled(scrollView, x, y, getRef());
                            }
                        }
                    }
                }
        ...
        host.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver
    ...
    protected boolean setProperty(String key, Object param) {
        switch (key) {
            ...
            case Constants.Name.OFFSET_ACCURACY:
                ...
                return true;
            case "pagingEnabled":
                boolean pagingEnable = WXUtils.getBoolean(param, false);
                setPagingEnable(pagingEnable);
                break;
        }
        return super.setProperty(key, param);
    }

    private void setPagingEnable(boolean pagingEnable) {
    ...
    public void setOffsetAccuracy(int accuracy) {
        float realPx = WXViewUtils.getRealPxByWidth(accuracy, getInstance()
    ...
    public void scrollTo(WXComponent component, Map<String, Object> options) {
        ...
        if (options != null) {
            String offset = options.get(Constants.Name.OFFSET) == null ? "0" : options.get
                    (Constants.Name.OFFSET).toString();
        ...
        scrollBy(viewXInScroller - getScrollX() + (int) offsetFloat, viewYInScroller - getScrollY
                () + (int) offsetFloat, smooth);
    }
    ...
    private void procAppear(int x, int y, int oldx,
        ...
        if (mOrientation == Constants.Orientation.HORIZONTAL && moveX != 0) {
            direction = moveX > 0 ? Constants.Value.DIRECTION_RIGHT : Constants.Value
                    .DIRECTION_LEFT;
        }
        ...
        for (Entry<String, AppearanceHelper> item : mAppearanceComponents.entrySet()) {
            ...
            if (result != AppearanceHelper.RESULT_NO_CHANGE) {
                helper.getAwareChild().notifyAppearStateChange(result == AppearanceHelper
                                .RESULT_APPEAR ? Constants.Event.APPEAR : Constants.Event.DISAPPEAR,
                        direction);
            }
        }
    }
    ...
    /**
     * Handle loadMore Event.when Scroller has bind loadMore Event and set the attr of
     * loadMoreOffset it will tell the JS to handle the event of onLoadMore;
     *
    ...
    protected void onLoadMore(WXScrollView scrollView, int x, int y) {
        try {
            ...
            int offsetInt = (int) WXViewUtils.getRealPxByWidth(Float.parseFloat(offset),
                    getInstance().getInstanceViewPortWidth());
