...
public abstract class WXComponent<T extends View> implements IWXObject, IWXActivityStateListener,
    ...
    private OnClickListener mClickEventListener = new OnClickListener() {
        ...
        public void onHostViewClick() {
            ...
            position.put("y", WXViewUtils.getWebPxByWidth(location[1], mInstance.getViewPortWidth
                    ()));
            position.put("width", WXViewUtils.getWebPxByWidth(mDomObj.getLayoutWidth(), mInstance
                    .getViewPortWidth()));
            position.put("height", WXViewUtils.getWebPxByWidth(mDomObj.getLayoutHeight(),
                    mInstance.getViewPortWidth()));
    ...
    public final void invoke(String method, JSONArray args) {
        ...
        if (invoker != null) {
            try {
                ...
                WXLogUtils.e("[WXComponent] updateProperties :" + "class:" + getClass() +
                        "method:" + invoker.toString() + " function " + WXLogUtils.getStackTrace
                        (e));
            }
        } else {
    ...
    /**
     * Will be invoked when request method not found. Subclass should override this method, If you
     * return hard-code method list in {@link IFComponentHolder#getMethods()}
     */
    ...
    @Deprecated
    public WXComponent(WXSDKInstance instance, WXDomObject dom, WXVContainer parent, String
            instanceId, boolean isLazy) {
        this(instance, dom, parent, isLazy);
    }

    @Deprecated
    public WXComponent(WXSDKInstance instance, WXDomObject dom, WXVContainer parent, boolean
            isLazy) {
    ...
    protected final void fireEvent(String type, Map<String, Object> params, Map<String, Object>
            domChanges) {
    ...
    public void applyLayoutAndEvent(WXComponent component) {
        if (!isLazy()) {
            ...
            Log.e("layout","applyLayoutAndEvent");
    ...
    public final void setLayout(ImmutableDomObject domObject) {
        ...
        if (mPreRealWidth == realWidth && mPreRealHeight == realHeight && mPreRealLeft ==
        ...
        if (!mInstance.mEnd && !(mHost instanceof ViewGroup) && mAbsoluteY + realHeight >
                mInstance.getWeexHeight() + 1) {
        ...
        Log.e("layout",getClass().getSimpleName()+"realWidth>>>"+realWidth+"realHeight>>>"+realHeight);
        ...
        if (mDomObj.isFixed()) {
            ...
            setHostLayoutParams(mHost, realWidth, realHeight, realLeft, realRight, realTop,
                    realBottom);
        }
    ...
    protected void setHostLayoutParams(T host, int width, int height, int left, int right, int
            top, int bottom) {
    ...
    private void setFixedHostLayoutParams(T host, int width, int height, int left, int right, int
            top, int bottom) {
        ...
        if (WXEnvironment.isApkDebugable()) {
            WXLogUtils.d("Weex_Fixed_Style", "WXComponent:setLayout :" + left + " " + top + " " +
                    width + " " + height);
            WXLogUtils.d("Weex_Fixed_Style", "WXComponent:setLayout Left:" + mDomObj.getStyles()
                    .getLeft() + " " + (int) mDomObj.getStyles().getTop());
        }
    }

    /**
     * After component's layout result is apply to view. May be invoke multiple times since DOM can
     * be changed in js runtime.
     */
            ...
            if (!setProperty(key, param)) {
                ...
                if (invoker != null) {
                    try {
                        ...
                        WXLogUtils.e("[WXComponent] updateProperties :" + "class:" + getClass() +
                                "method:" + invoker.toString() + " function " + WXLogUtils
                                .getStackTrace(e));
                    }
                }
            }
        ...
        if (type.equals(Constants.Event.CLICK) && view != null) {
            ...
            if (view instanceof WXGestureObservable) {
                if (mGesture == null) {
                    ...
                    boolean isPreventMove = WXUtils.getBoolean(getDomObject().getAttrs().get
                            (Constants.Name.PREVENT_MOVE_EVENT), false);
    ...
    /**
     * Judge whether need to set an onTouchListener.<br> As there is only one onTouchListener in
     * each view, so all the gesture that use onTouchListener should put there.
     *
    ...
    /**
     * Called after host view init. <br> Any overriding methods should invoke this method at the
     * right time, to ensure the cached animation can be triggered correctly. (the animation will be
     * cached when {@link #isLazy()} is true)
     *
    ...
    protected void removeEventFromView(String type) {
        if (type.equals(Constants.Event.CLICK) && getRealView() != null && mHostClickListeners !=
                null) {
    ...
    public void setBackgroundImage(String bgImage) {
        if (!TextUtils.isEmpty(bgImage) && mHost != null) {
            Shader shader = WXResourceUtils.getShader(bgImage, mDomObj.getLayoutWidth(), mDomObj
                    .getLayoutHeight());
            ...
            switch (key) {
                case Constants.Name.BORDER_RADIUS:
                    getOrCreateBorder().setBorderRadius(BorderDrawable.BORDER_RADIUS_ALL,
                            WXViewUtils.getRealSubPxByWidth(borderRadius, mInstance
                                    .getViewPortWidth()));
                    break;
                case Constants.Name.BORDER_TOP_LEFT_RADIUS:
                ...
                case Constants.Name.BORDER_TOP_RIGHT_RADIUS:
                    ...
                    break;
                case Constants.Name.BORDER_BOTTOM_RIGHT_RADIUS:
                    getOrCreateBorder().setBorderRadius(BorderDrawable
                            .BORDER_BOTTOM_RIGHT_RADIUS, WXViewUtils.getRealSubPxByWidth
                            (borderRadius, mInstance.getViewPortWidth()));
                    break;
                case Constants.Name.BORDER_BOTTOM_LEFT_RADIUS:
    ...
    public void setBorderWidth(String key, float borderWidth) {
        if (borderWidth >= 0) {
            switch (key) {
                case Constants.Name.BORDER_WIDTH:
                    getOrCreateBorder().setBorderWidth(Spacing.ALL, WXViewUtils
                            .getRealSubPxByWidth(borderWidth, getInstance().getViewPortWidth()));
                    break;
                case Constants.Name.BORDER_TOP_WIDTH:
                    getOrCreateBorder().setBorderWidth(Spacing.TOP, WXViewUtils
                            .getRealSubPxByWidth(borderWidth, getInstance().getViewPortWidth()));
                    break;
                case Constants.Name.BORDER_RIGHT_WIDTH:
                    getOrCreateBorder().setBorderWidth(Spacing.RIGHT, WXViewUtils
                            .getRealSubPxByWidth(borderWidth, getInstance().getViewPortWidth()));
                    break;
                case Constants.Name.BORDER_BOTTOM_WIDTH:
                    getOrCreateBorder().setBorderWidth(Spacing.BOTTOM, WXViewUtils
                            .getRealSubPxByWidth(borderWidth, getInstance().getViewPortWidth()));
                    break;
                case Constants.Name.BORDER_LEFT_WIDTH:
                    getOrCreateBorder().setBorderWidth(Spacing.LEFT, WXViewUtils
                            .getRealSubPxByWidth(borderWidth, getInstance().getViewPortWidth()));
                    break;
            }
        }
    ...
    /**
     * Detach view from its component. Components, which have difference between getHostView and
     * getRealView or have temp calculation results, must<strong> override</strong>  this method
...
//    mHost = null;
    ...
    /**
     * This method computes user visible left-top point in view's coordinate. The default
     * implementation uses the scrollX and scrollY of the view as the result, and put the value in
     * the parameter pointer. Components with different computation algorithm <strong> should
     * override </strong> this method.
     *
    ...
    private void setActiveTouchListener() {
        boolean hasActivePesudo = mDomObj.getStyles().getPesudoStyles().containsKey(Constants
                .PSEUDO.ACTIVE);
        View view;
        if (hasActivePesudo && (view = getRealView()) != null) {
            boolean hasTouchConsumer = (mHostClickListeners != null && mHostClickListeners.size()
                    > 0) || mGesture != null;
